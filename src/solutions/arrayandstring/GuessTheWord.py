"""
### GOOGLE MOST FREQ ASKED QUESTIONS ###
...
Time Complexity: O(N^2 log N), where NN is the number of words, and assuming their length is O(1). Each call to solve is O(N^2) and the number of calls is bounded by O(\log N)O(logN).
