"""
A Mars rover is directed to move within a square matrix. It accepts a sequence of commands to move in
any of the four directions from each cell: [UP, DOWN, LEFT or RIGHT]. The rover starts from cell 0. 
...
def get_test_case():
    #return 4, ["RIGHT", "UP", "DOWN","LEFT", "DOWN", "DOWN"], 12
    return 4, ['RIGHT', 'DOWN', 'LEFT', 'LEFT', 'DOWN'], 8 
...
def rover_move(n, cmds):
    pos = [0,0]
    for cmd in cmds:
        if cmd == "LEFT":
            move_pos(pos, [0,-1], size)
        elif cmd == "UP":
            move_pos(pos, [-1,0], size)
        if cmd == "RIGHT":
            move_pos(pos, [0,1], size)
        if cmd == "DOWN":
            move_pos(pos, [1,0], size)
...
def move_pos(pos, direction, size):
    column = pos[1] + direction[1] 
    row = pos[0] + direction[0] 
    if column >= 0 and column < size and row >= 0 and row < size:
        pos[1] = column
        pos[0] = row 
